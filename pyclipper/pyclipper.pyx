...
cdef extern from "clipper.hpp" namespace "ClipperLib":
    ...
    cdef cppclass PolyTree(PolyNode) nogil:
        PolyTree()
        PolyNode& GetFirst()
        void Clear()
        int Total()
    ...
    cdef cppclass ClipperOffset nogil:
    ...
    void c_PolyTreeToPaths "PolyTreeToPaths"(const PolyTree& polytree, Paths& paths) nogil
    void c_ClosedPathsFromPolyTree "ClosedPathsFromPolyTree"(const PolyTree& polytree, Paths& paths) nogil
    void c_OpenPathsFromPolyTree "OpenPathsFromPolyTree"(PolyTree& polytree, Paths& paths) nogil
...
def Orientation(poly):
    ...
    cdef bint orientation
    with nogil:
        orientation = c_Orientation(c_path)
    return orientation
...
def PointInPolygon(point, poly):
    ...
    cdef int isinside
    with nogil:
        isinside = c_PointInPolygon(c_point, c_path)
    return isinside
...
def SimplifyPolygons(polys, PolyFillType fill_type=pftEvenOdd):
    ...
    """
    cdef Paths out_polys
    cdef Paths c_paths = _to_clipper_paths(polys)
    with nogil:
        c_SimplifyPolygons(c_paths, out_polys, fill_type)
    return _from_clipper_paths(out_polys)
...
def MinkowskiSum(pattern, path, bint path_is_closed):
    ...
    """
    cdef Paths solution
    cdef Path c_pattern = _to_clipper_path(pattern)
    cdef Path c_path = _to_clipper_path(path)
    with nogil:
        c_MinkowskiSum(c_pattern, c_path, solution, path_is_closed)
...
def MinkowskiSum2(pattern, paths, bint path_is_closed):
    ...
    with nogil:
        c_MinkowskiSum(c_pattern, c_paths, solution, path_is_closed)
    return _from_clipper_paths(solution)
...
cdef class Pyclipper:
    ...
    def AddPath(self, path, PolyType poly_type, closed=True):
        ...
        cdef Path c_path = _to_clipper_path(path)
        cdef bint result
        cdef bint c_closed = closed
        with nogil:
            result = self.thisptr.AddPath(c_path, poly_type, c_closed)
        if not result:
    ...
    def AddPaths(self, paths, PolyType poly_type, closed=True):
        ...
        cdef bint c_closed = closed
        with nogil:
            result = self.thisptr.AddPaths(c_paths, poly_type, c_closed)
        if not result:
    ...
    def Execute(self, ClipType clip_type,
        ...
        cdef Paths solution
        cdef bint success  # used to be type object. Not sure why
        with nogil:
            success = self.thisptr.Execute(clip_type, solution, subj_fill_type, clip_fill_type)
        if not success:
